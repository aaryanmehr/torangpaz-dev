document.addEventListener("DOMContentLoaded", async () => {                                  // 📌 اجرای کد پس از بارگذاری کامل DOM
  ...
  try {
    ...
    if (!tabsHTML.length) {                                                                 // 📌 اگر تب قابل نمایشی نبود
      ...
      // 📌 اجرای تنظیمات برای تب‌هایی که بعداً فعال می‌شوند (برای حل مشکل سایز تصاویر)
      const tabLinks = menuTabsContainer.querySelectorAll(".nav-link");

      tabLinks.forEach(link => {
        link.addEventListener("shown.bs.tab", () => {                                       // 📌 هنگامی که یک تب فعال می‌شود
          if (typeof AOS !== "undefined") AOS.refresh();                                    // 📌 بازنشانی انیمیشن‌ها

          const activePane = document.querySelector(".tab-pane.active.show");              // 📌 تب فعال‌شده فعلی
          const imgs = activePane?.querySelectorAll("img");                                // 📌 تصاویر داخل تب فعال

          imgs?.forEach(img => {
            img.style.display = "none";
            void img.offsetHeight;                                                         // 📌 اعمال reflow برای بازنشانی سایز تصویر
            img.style.display = "";
          });
        });
      });
